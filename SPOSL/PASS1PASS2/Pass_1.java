...
public class Pass_1 {
	...
	public static void main(String[] args) throws IOException{
        ...
        try{
            ...
            BufferedWriter bw = new BufferedWriter(new FileWriter("src\\asm\\intermediate.txt"));
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    switch (words[i]) {
                        ...
                        case "LTORG":
                            ltorg = true;
                            pos = 5;
                            //We need to process all literals occurred before LTORG statement, so we use total_ltr to maintain count of literals
                            for (temp = 0; temp < total_ltr; temp++)
                                if (literal_table[temp].address == 0) {
                                    literal_table[temp].address = loc; //Assign the ongoing line address to literals
                    ...
                    if (pos == -1) {
                        //STEP 5.1 - Check whether given word is an mnemonic by checking OP Table.
                        ...
                        if (pos != -1) {
                            ...
                            op_table[optab_cnt++] = new Tables(words[i], pos); //Update its entry in MOT
                        }
...
//                                    pos = total_symb;
            ...
            BufferedWriter sw = new BufferedWriter(new FileWriter("src\\asm\\symTab.txt"));
            sw.write("\nSYMBOL\tADDRESS\n");
            for(int i=0;i<total_symb;i++)
                sw.write(symbol_table[i].name+"\t\t"+symbol_table[i].address+"\n");
            sw.close();

            BufferedWriter lw = new BufferedWriter(new FileWriter("src\\asm\\litTab.txt"));
            ...
            BufferedWriter pw = new BufferedWriter(new FileWriter("src\\asm\\poolTab.txt"));
            pw.write("\nPOOL\tTOTAL LITERALS\n");
            for(int i=0;i<pooltab_cnt;i++)
                pw.write(poolTab[i].first+"\t\t\t"+poolTab[i].total_literals+"\n");
            pw.close();

            BufferedWriter mw = new BufferedWriter(new FileWriter("src\\asm\\opTab.txt"));
            mw.write("\nMNEMONIC\tOPCODE\n");
